...
QString UrlFactory::MakeImageUrl(const MapType::Types &type, const Point &pos, const int &zoom, const QString &language)
{
    ...
    case MapType::GoogleSatellite:
    {
        ...
        GetSecGoogleWords(pos, sec1, sec2);
        TryCorrectGoogleVersions();
        QString s = QString("http://%1%2.google.com/%3/v=%4&hl=%5&x=%6%7&y=%8&z=%9&s=%10").arg(server).arg(GetServerNum(pos, 4)).arg(request).arg(VersionGoogleSatellite).arg(language).arg(pos.X()).arg(sec1).arg(pos.Y()).arg(zoom).arg(sec2);
        printf("URL: %s\n", s.toStdString().c_str());
        return s;
    }
    break;
